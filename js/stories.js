...
function generateStoryMarkup(story, showDeleteBtn = false) {
  ...
  // show/hide star based on user logged in status
...
// new story form submission handler
...
function putFavoritesListOnPage() {
  ...
  if (currentUser.favorites.length === 0) {
    ...
    for (let story of currentUser.favorites) {
      const $story = generateStoryMarkup(story);
      $favoriteStories.append($story); // append all favorited stories to the </ol>
    }
  }
  $favoriteStories.show(); // show only starred stories in the page
}
...
function putMyStoriesListOnPage() {
  ...
  if (currentUser.ownStories.length === 0) {
    ...
    for (let ownStory of currentUser.ownStories) {
      ...
      $ownStories.append($ownStory) //append all user stories to the </ol>
    }
  }
  $ownStories.show(); // only show user's stories in the page
}
